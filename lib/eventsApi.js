...
export const ENDPOINTS = {
  ...
  photos: (id) => `/api/events/${id}/photos`, // 👈 galería
};
...
// Listar fotos de la galería del evento
export async function fetchEventPhotos(id) {
  const r = await api(ENDPOINTS.photos(id), { method: 'GET' });
  if (!r.ok) return r;

  // backend típico: { photos: [ {url, byUsername?, uploadedAt?}, ... ] }
  const listRaw = Array.isArray(r.data?.photos)
    ? r.data.photos
    : Array.isArray(r.data)
    ? r.data
    : [];

  const list = listRaw.map(p => (typeof p === 'string' ? { url: p } : p));
  return { ok: true, status: 200, data: list };
}

/**
 * Borrar una foto del evento (solo propietario).
 * Puedes pasar:
 *  - { idx: number }    -> borra por posición en el array
 *  - { url: string }    -> borra por URL (relativa o absoluta)
 *  - { pid: string }    -> se usa como segmento de ruta /photos/:pid
 */
export async function deleteEventPhoto(id, opts = {}) {
  const token = getToken();

  // Construimos la URL: preferimos pid (segmento), si no, query (?idx= | ?url=)
  let endpoint = ENDPOINTS.photos(id);
  const qs = new URLSearchParams();

  if (typeof opts.pid === 'string' && opts.pid.trim() !== '') {
    endpoint = `${ENDPOINTS.photos(id)}/${encodeURIComponent(opts.pid)}`;
  } else if (Number.isInteger(opts.idx)) {
    qs.set('idx', String(opts.idx));
